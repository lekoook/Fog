...
class Bluetoothctl:
    ...
    def get_output(self, command, pause = 0):
        ...
        start_failed = self.child.expect(["(\x1b\\[0;94m)\\[.+\\](\x1b\\[0m)", pexpect.EOF])
    ...
    def isConnected(self, mac_address):
        """ Check if a device is connected. """
        lines = self.get_device_info(mac_address)
        
        if lines is not None:
            # Check for the line that says: "Connected: yes"
            for l in lines:
                if "Connected: yes" in l:
                    return True
            return False
        else:
            return False
