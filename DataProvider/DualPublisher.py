...
def pubMock(publisher: zmq.Socket, topic: str, filePath: str):
    ...
    for root, dirs, files in os.walk(config.MOCK_DATA_FOLDER, topdown=False):
        for folder in dirs:
            os.chdir('/home/pi/Fog-master/DataProvider/' + os.path.join(root, folder))
            print(os.getcwd())
    ...
    #Creating L and R feet data streams        
            ...
            while True:
                try:
                    ...
                    # Publish onto topic
                    publisher.send_string("%s %i %i %i %i %i %i %i" % (topic, lax, lay, laz, rax, ray ,raz, gt))
                    print("%s %i %i %i %i %i %i %i" % (topic, lax, lay, laz, rax, ray ,raz, gt))
